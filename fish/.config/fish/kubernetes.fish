...
function kl
    set ns (test -n "$argv[1]"; and echo "$argv[1]"; or kubectl config view --minify --output 'jsonpath={..namespace}')
    set pods (kubectl get pods -n $ns -o name | sed 's/^pod\///')
    if test -z "$pods"
        echo "No pods found in namespace '$ns'."
        return
    end
    set pod (echo $pods | tr ' ' '\n' | fzf)
    if test -n "$pod"
