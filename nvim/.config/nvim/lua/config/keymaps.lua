...
vim.keymap.set('n', '<leader>ch', function()
  ...
  local cmd = string.format("tmux new-window -c '%s' -n 'opencode' \\; send-keys 'opencode' Enter", cwd)
  os.execute(cmd)
end, { desc = 'Open tmux tab with opencode' })
vim.keymap.set('n', '<leader>cd', function()
  local cwd = vim.fn.getcwd()
  local cmd = string.format("tmux split-window -h -p 25 -c '%s' \\; send-keys 'opencode' Enter", cwd)
  os.execute(cmd)
end, { desc = 'Open tmux split with opencode' })
vim.keymap.set('n', '<leader>ocr', function()
  local cwd = vim.fn.getcwd()
  local cmd = string.format("tmux new-window -c '%s' -n 'opencode run' \\; send-keys 'opencode run \"\"' \\; send-keys Left", cwd)
  os.execute(cmd)
end, { desc = 'Open tmux tab with opencode run' })
