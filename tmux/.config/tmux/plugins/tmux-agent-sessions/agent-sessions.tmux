...
agent_title='#{s/ - [^ ]+$//;s/^π - //;s/^OC [|] //;s/^[✳*] *//:pane_title}'
agent_format="#{?pane_format,$agent_title,#{?window_format,$agent_title,#{session_windows} windows#{?session_attached, (attached),}}}"
...
if [ "$ui" = 'choose-tree' ]; then
  tmux bind-key "$key" choose-tree -Z -f "$agent_filter" -F "$agent_format" "switch-client -t '%%'"
else
  tmux bind-key "$key" display-popup -E -T ' agent sessions ' -w "$width" -h "$height" "$SCRIPT popup"
fi

tmux set-hook -g client-session-changed "run-shell -b '$SCRIPT cache-refresh'"
tmux set-hook -g session-created "run-shell -b '$SCRIPT cache-refresh'"
tmux set-hook -g session-closed "run-shell -b '$SCRIPT cache-refresh'"
tmux set-hook -g pane-exited "run-shell -b '$SCRIPT cache-refresh'"
tmux set-hook -g window-renamed "run-shell -b '$SCRIPT cache-refresh'"
tmux run-shell -b "$SCRIPT cache-refresh"
