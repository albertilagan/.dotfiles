#!/usr/bin/env bash
# Toggle a dedicated opencode pane (show/hide without killing)
PANE_ID=$(tmux show-environment -g OPENCODE_PANE 2>/dev/null | sed 's/OPENCODE_PANE=//')
...
if [ -n "$PANE_ID" ] && tmux list-panes -a -F '#{pane_id}' | grep -qF "$PANE_ID"; then
  ...
  if [ "$PANE_WIN" = "$CUR_WIN" ]; then
    tmux break-pane -d -s "$PANE_ID" -n _opencode_hidden
  else
  ...
  tmux set-environment -g OPENCODE_PANE "$(tmux display-message -p '#{pane_id}')"
fi
